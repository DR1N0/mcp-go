...
func main() {
	fmt.Println("=================================================================================")
	fmt.Println("MCP Go Client Example (gRPC Transport)")
	fmt.Println("=================================================================================")
	fmt.Println("This example demonstrates connecting a Go client to an MCP server via gRPC")
	fmt.Println()
	...
	transport := grpc.NewClientTransport(serverAddr)
	...
	echoArgs := map[string]interface{}{
		"message": "Hello from Go gRPC client!",
	}
	...
	// Call weather tool
	fmt.Println("\n[5] Calling 'get_weather' tool...")
	weatherArgs := map[string]interface{}{
		"latitude":  37.7749,
		"longitude": -122.4194,
	}
	weatherResp, err := client.CallTool(ctx, "get_weather", weatherArgs)
	if err != nil {
		log.Fatalf("Failed to call weather tool: %v", err)
	}

	if len(weatherResp.Content) > 0 && weatherResp.Content[0].Text != nil {
		textPreview := *weatherResp.Content[0].Text
	...
	if len(resourcesResp.Resources) > 0 {
		fmt.Println("\n[9] Reading 'config://server' resource...")
	...
	// Ping test
	fmt.Println("\n[10] Testing server connectivity...")
