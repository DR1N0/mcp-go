#!/usr/bin/env python3
"""
SSE transport test for mcp-go server using pydantic_ai.

This script tests the MCP server implementation by:
1. Connecting to the Go server via SSE
...
load_dotenv()
SERVER_URL = os.getenv("MCP_SSE_URL", "http://localhost:8001/mcp/sse")
...
async def test_call_echo_tool(server):
    ...
    result = await server.direct_call_tool(
        "echo",
        {"message": "Hello from Python SSE client!"}
    )
    
    assert isinstance(result, str), f"Expected string result, got {type(result)}"
    assert "Hello from Python SSE client!" in result
...
async def test_read_config_resource(server):
    ...
    assert config["transport"] == "sse", "Transport should be 'sse'"
...
async def run_tests():
    """Run all async tests."""
    print("=" * 80)
    print("MCP-Go SSE Transport Tests (Python Client)")
    print("=" * 80)
    print(f"Server URL: {SERVER_URL}")
    
    # Create MCP connection via SSE
    server = MCPServerSSE(
        url=SERVER_URL,
        tool_prefix=""
    )
    ...
    try:
        ...
        # Server connection will be closed automatically
        pass
