...
import (
	...
	"github.com/DR1N0/mcp-go/transport/sse"
)
...
func main() {
	...
	server := mcpgo.NewServer(
		sse.NewServerTransport("/mcp/sse", ":8001"),
		mcpgo.WithName("sse-example-server"),
	...
	log.Println("   - SSE endpoint: http://localhost:8001/mcp/sse (GET)")
	log.Println("     * Client connects here to receive server messages")
	log.Println("     * Server sends 'endpoint' event with message URL")
	log.Println("   - Message endpoint: http://localhost:8001/mcp/sse/message?session_id=... (POST)")
	log.Println("     * Client sends JSON-RPC requests here")
	log.Println("   - Health check: http://localhost:8001/mcp/sse/health")
	log.Println("   - Tools: echo, add")
