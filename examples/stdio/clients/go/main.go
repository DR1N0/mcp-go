...
func main() {
	fmt.Println("=================================================================================")
	fmt.Println("MCP Go Client Example (Stdio Transport)")
	fmt.Println("=================================================================================")
	fmt.Println("This example demonstrates connecting a Go client to an MCP server via stdio")
	fmt.Println()

	// Path to the stdio server
	serverPath := "go"
	serverArgs := []string{"run", "./examples/stdio/server/main.go"}
	transport := stdio.NewClientTransport(serverPath, serverArgs)
	...
	echoArgs := map[string]interface{}{
		"message": "Hello from Go stdio client!",
	}
	...
	// Close client
	fmt.Println("\n[10] Closing client...")
	// Give server a moment to process final messages
